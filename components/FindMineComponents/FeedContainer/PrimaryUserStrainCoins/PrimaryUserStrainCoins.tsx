...
import { TOGGLE_SHOW_INPUT, SET_FEED_SELECTED_USER, SET_COIN_HOVER_STRAIN, SET_FEED_SELECTED_USER_REVIEWS } from "redux/findMine/findMineSlice"
...
export default function PrimaryUserStrainCoins ({allStrainsForAllUsers, allLikesFromAllUsers, allReviewsFromAllUsers}) {
    return <RENDER allStrainsForAllUsers={allStrainsForAllUsers} allLikesFromAllUsers={allLikesFromAllUsers} allReviewsFromAllUsers={allReviewsFromAllUsers} />
}

function RENDER(props:any) {
    const dispatch = useDispatch()
    const { coin } = useImage()
    const { userLikeOrNotPROMISE } = usePromise()
    ...
    // pages/findMine.tsx ------> PROP DRILLING DB DATA FROM GETSERVERSIDEPROPS 
    const allStrainsForAllUsers = props.allStrainsForAllUsers
    const allLikesFromAllUsers = props.allLikesFromAllUsers
    const allReviewsFromAllUsers = props.allReviewsFromAllUsers
    ...
    const clickCoin = (item, event) => {
        ...
        userLikeOrNotPROMISE(event, id, allLikesFromAllUsers)
        
        console.log('feed selected user', FEED_SELECTED_USER)

        console.log('allReviews', allReviewsFromAllUsers)


        const clickedStrainUserReview = allReviewsFromAllUsers.find(reviews => reviews.userId === FEED_SELECTED_USER.id && reviews.strainid === id)
        console.log('clicked strain review', clickedStrainUserReview)
        if (clickedStrainUserReview) {
            dispatch(SET_FEED_SELECTED_USER_REVIEWS(clickedStrainUserReview))
        } else {
            dispatch(SET_FEED_SELECTED_USER_REVIEWS({ title: '', userId: 0, review: '', strainid: 0 }))
        }



        // const userLikeOrNotPROMISE = (event, userID:number, allLikes:any) => {
    }
