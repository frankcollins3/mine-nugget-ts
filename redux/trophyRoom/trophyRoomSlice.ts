...
interface TrophyRoomSliceState {
  ...
  // STARS_SEARCH_TERM: string;
  // ALL_USERS_SEARCH_TERM_MEET: any[];
...
const trophyRoomSlice = createSlice({
  ...
  reducers: {
    ...
    SET_STARS: (state, action) => { state.STARS.push(action.payload) }, // SET_STARS: (state, action) => { state.STARS = action.payload },
